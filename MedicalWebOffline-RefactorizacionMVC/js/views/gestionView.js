...
export async function renderGestionModulos(container) {
    try {
        ...
        let contenidoTabla;
        if (modulos.length === 0) {
            contenidoTabla = '<div class="empty-message">No hay módulos registrados. Crea uno nuevo para comenzar.</div>';
        } else {
            contenidoTabla = renderTablaModulos(modulos, grupos);
        }
        ...
        // Configurar evento para nuevo módulo
        ...
        container.innerHTML = `
            ...
            ">
                ...
                <strong>Error al cargar módulos</strong>
                <p>${error.message}</p>
...
export async function renderGestionGrupos(container) {
    try {
        ...
        let contenidoTabla;
        if (grupos.length === 0) {
            contenidoTabla = '<div class="empty-message">No hay grupos registrados. Crea uno nuevo para comenzar.</div>';
        } else {
            contenidoTabla = await renderTablaGrupos(grupos);
        }
        ...
        if (tablaGrupos) {
            ...
            nuevaTabla.addEventListener('click', async (e) => {
                ...
                if (target.classList.contains('assign-practicantes')) {
                    ...
                    await mostrarModalAsignarPracticantes(grupoId);
                }
                    ...
                    gestionModel.getGrupoById(grupoId).then(grupo => {
                        ...
                        modalUtil.confirmarAccion({
                            ...
                            onConfirm: async () => {
                                try {
                                    ...
                                    eventBus.emit(EVENT_NAMES.GROUP_DELETED, { group: grupo });
        ...
        container.innerHTML = `
            ...
            ">
                <i class="fas fa-exclamation-triangle" style="font-size: 24px; margin-bottom: 12px; display: block;"></i>
                <strong>Error al cargar grupos</strong>
                <p>${error.message}</p>
                <button onclick="location.reload()" style="
                    background: #dc2626;
                    color: white;
...
async function renderTablaGrupos(grupos) {
    try {
            ...
            <table class="data-table">
                ...
                <tbody>
                    ${grupos.map(grupo => {
                        ...
                        console.log(`Debug - Grupo ${grupo.nombre} (${grupo.id}):`);
                        console.log('- Usuarios totales:', todosLosUsuarios.length);
                        console.log('- Practicantes con grupoId:', todosLosUsuarios.filter(u => u.rol === 'practicante' && u.grupoId).length);
                        console.log('- Practicantes en este grupo:', miembrosReales);
                        console.log('- Usuarios del grupo:', todosLosUsuarios.filter(u => u.grupoId === grupo.id));
        ...
        console.error('Error renderizando tabla de grupos:', error);
        return `<div class="error-message">Error cargando tabla de grupos: ${error.message}</div>`;
    }
}
...
async function mostrarModalAsignarPracticantes(grupoId) {
    ...
    modalContainer.addEventListener('click', async (e) => {
        if (e.target.closest('.btn-remove-practicante')) {
            ...
            if (!usuario) {
                console.error('Usuario no encontrado para ID:', usuarioId);
            ...
            modalUtil.confirmarAccion({
                ...
                onConfirm: async () => {
                    try {
                        // Actualizar el usuario quitando su grupoId directamente en Firebase
                        const updateResult = await authModel.updateUser(usuarioId, { grupoId: null });
                        
                        if (updateResult) {
                            // También quitar el usuario del array miembros del grupo
                            await gestionModel.quitarUsuarioDeGrupo(grupoId, usuarioId);
                        ...
                        alert('Error al remover usuario del grupo: ' + error.message);
                    }
                }
            });
        }
    ...
    guardarBtn.addEventListener('click', async () => {
        ...
        console.log('GestionView: Checkboxes encontrados:', checkboxes.length);
        console.log('GestionView: Practicantes seleccionados:', practicantesSeleccionados);
        ...
        for (const usuarioId of practicantesSeleccionados) {
            console.log('GestionView: Procesando usuario ID:', usuarioId);
            try {
                // Actualizar el usuario directamente con Firebase
                const updateResult = await authModel.updateUser(usuarioId, { grupoId: grupoId });
                
                if (updateResult) {
                    // Ahora también agregar el usuario al array miembros del grupo
                    await gestionModel.asignarUsuarioAGrupo(grupoId, usuarioId);
                    
                    console.log('GestionView: Usuario', usuarioId, 'asignado exitosamente al grupo', grupoId);
                    asignacionesExitosas.push({ id: usuarioId });
                    ...
                    console.error('GestionView: No se pudo actualizar usuario', usuarioId);
                    asignacionesFallidas.push({ id: usuarioId, error: 'No se pudo actualizar en Firebase' });
                }
            } catch (error) {
                console.error(`Error asignando usuario ${usuarioId} al grupo:`, error);
                asignacionesFallidas.push({ id: usuarioId, error: error.message });
            }
        }
        ...
        if (asignacionesExitosas.length > 0) {
            modalUtil.mostrarAlerta({
                ...
                message: `Se han asignado ${asignacionesExitosas.length} practicante(s) al grupo "${grupo.nombre}"`,
                type: 'success'
            });
        }
        ...
        console.error('Error en mostrarModalAsignarPracticantes:', error);
        alert('Error cargando el modal: ' + error.message);
    }
}

// Función de debug para consola
window.debugGruposUsuarios = async function() {
    console.log('=== DEBUG GRUPOS Y USUARIOS ===');
    ...
    try {
        ...
        const practicantes = usuarios.filter(u => u.rol === 'practicante');
        console.log('Practicantes encontrados:', practicantes);
        
        grupos.forEach(grupo => {
            const miembros = usuarios.filter(u => u.grupoId === grupo.id);
            console.log(`Grupo ${grupo.nombre}: ${miembros.length} miembros`, miembros);
        });
    } catch (error) {
...
console.log('GestionView: Módulo cargado');
console.log('💡 Para debug de grupos y usuarios, ejecuta: debugGruposUsuarios()');